...
/*
 * Author: Kduckp
 * Generated on: 2025-07-11 11:17:09
*/
...
__TOISETHIVOI__ {
    ...
    FOR(i, n) {
        ll tuition = c[i];
        ll count = n - i; // số lượng bò sẽ đóng tiền >= tuition
        ll revenue = tuition * count;

        if (revenue > max_money) {
            max_money = revenue;
            best_tuition = tuition;
        } else if (revenue == max_money) {
            best_tuition = min(best_tuition, (int)tuition);
        }
    }
