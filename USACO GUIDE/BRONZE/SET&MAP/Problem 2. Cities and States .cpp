...
void djk(int n, int start, int end, vector<road>& a, int k) {
    vector<vector<addition>> adj(n + 1);
    for (int i = 0; i < (int)a.size(); ++i) {
        road r = a[i];
        if(r.h < k)
        {
            adj[r.u].push_back({r.v, r.t, r.h});
            adj[r.v].push_back({r.u, r.t, r.h});
        }
    }
    ...
    ll res = INF;
    for(auto& i : dist[end])res = min(res, i);
...
__TOISETHIVOI__ {
    ...
    cin >> k >> n >> m;
    vector<road> a(m);
    for (int i = 0; i < m; i++)cin >> a[i].u >> a[i].v >> a[i].t >> a[i].h;
    int start, end;
    cin >> start >> end;
