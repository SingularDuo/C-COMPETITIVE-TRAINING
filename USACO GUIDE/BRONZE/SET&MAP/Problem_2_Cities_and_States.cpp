...
/*
 * Author: Kduckp
 * Generated on: 2025-07-13 22:22:38
*/
...
void init() {
    fast;
    name("citystate.in", "citystate.out");
}
...
__TOISETHIVOI__ {
    ...
    FOR(i, n) {
        ...
        freq[make_pair(prefix, state)]++;
    }

    int res = 0;
    for (map<pair<string, string>, int>::iterator it = freq.begin(); it != freq.end(); ++it) {
        string a = it->first.first;
        string b = it->first.second;

        pair<string, string> rev = make_pair(b, a);
        if (rev != it->first && freq.count(rev)) {
            res += it->second * freq[rev];
        }
    }
