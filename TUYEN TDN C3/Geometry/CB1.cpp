...
/*
 * Author: Kduckp
 * Generated on: 2025-07-14 13:11:20
*/
...
__TOISETHIVOI__ {
    ...
    point m,a,b;
    cin>>m.x>>m.y>>a.x>>a.y>>b.x>>b.y;
    int value = (a.y - b.y) * m.x + (b.x - a.x) * m.y + (a.x * b.y - b.x * a.y);


    if (value == 0) {

        if (m.x >= min(a.x, b.x) && m.x <= max(a.x, b.x) &&  m.y >= min(a.y, b.y) && m.y <= max(a.y, b.y)) {
            cout << " doan thang AB" << endl;
        }

        else if ((m.x - a.x) * (b.x - a.x) + (m.y - a.y) * (b.y - a.y) == 0) {
            cout << "tia AB" << endl;
        }
    }
    else no;
